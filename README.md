<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:00C9FF,45:7F00FF,100:FF0080&height=220&section=header&text=Rasesh%20Varshney&fontSize=58&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=Full-Stack%20Developer%20%E2%80%A2%20AI%20Builder%20%E2%80%A2%20Systems%20Thinker&descSize=18&descAlignY=58" alt="Rasesh Varshney animated header" />

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&duration=2800&pause=900&color=00D9FF&center=true&vCenter=true&width=760&lines=Building+products+that+feel+effortless;Engineering+systems+that+scale;Turning+AI+ideas+into+real+experiences;Always+learning.+Always+shipping." alt="Animated typing introduction" />
</a>

<img width="92%" src="https://raw.githubusercontent.com/rasesh13/rasesh13/main/assets/developer-orbit.svg" alt="Animated developer orbit" />

<p>
  <a href="https://github.com/rasesh13?tab=followers"><img src="https://img.shields.io/github/followers/rasesh13?style=for-the-badge&logo=github&label=Followers&color=7F00FF" alt="GitHub followers" /></a>
  <a href="https://github.com/rasesh13"><img src="https://komarev.com/ghpvc/?username=rasesh13&style=for-the-badge&color=00C9FF" alt="Profile views" /></a>
</p>

</div>

## `> whoami`

I am a full-stack developer focused on building **AI-powered products**, **scalable backend systems**, and **high-quality web experiences**. I enjoy taking ambitious ideas from a blank page to a deployed product—balancing engineering depth with thoughtful design.

- 🤖 Building practical AI experiences that help people learn and create
- ⚙️ Designing reliable APIs, data layers, and production-ready services
- 🎨 Crafting fast, responsive interfaces with strong attention to detail
- 🚀 Obsessed with shipping, measuring, learning, and improving

## Tech constellation

<div align="center">

<img src="https://skillicons.dev/icons?i=ts,js,python,react,nextjs,nodejs,fastapi,postgres,redis,tailwind,git,github,vercel&perline=7" alt="TypeScript, JavaScript, Python, React, Next.js, Node.js, FastAPI, PostgreSQL, Redis, Tailwind CSS, Git, GitHub, and Vercel" />

</div>

## Mission control

<table>
  <tr>
    <td align="center" width="33%">
      <h3>🧠 THINK</h3>
      <p>Product strategy<br/>System design<br/>AI-first workflows</p>
    </td>
    <td align="center" width="33%">
      <h3>⚡ BUILD</h3>
      <p>Full-stack products<br/>Scalable APIs<br/>Delightful interfaces</p>
    </td>
    <td align="center" width="33%">
      <h3>🚀 SHIP</h3>
      <p>Production quality<br/>Fast feedback loops<br/>Continuous improvement</p>
    </td>
  </tr>
</table>

## Featured builds

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🗣️ AI Pronunciation Coach</h3>
      <p>A free pronunciation coach with slow and natural speech, retry scoring, and recurring-mistake tracking.</p>
      <p><strong>Next.js • React • TypeScript • Python • AI</strong></p>
      <a href="https://github.com/rasesh13/english-ai-pronunciation-coach">Source</a> ·
      <a href="https://english-ai-pronunciation-coach.vercel.app">Live experience</a>
    </td>
    <td width="50%" valign="top">
      <h3>🏛️ Atlas Platform Backend</h3>
      <p>A production-grade API foundation built for security, observability, performance, and growth.</p>
      <p><strong>TypeScript • Fastify • PostgreSQL • Redis • Drizzle</strong></p>
      <a href="https://github.com/rasesh13/atlas-platform-backend">Explore the architecture</a>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>🥤 Coco Carvan</h3>
      <p>A premium digital storefront for an Aligarh cold-cocoa brand, built as a polished modern web experience.</p>
      <p><strong>Next.js • React • TypeScript • Tailwind CSS</strong></p>
      <a href="https://github.com/rasesh13/cococarvan">Source</a> ·
      <a href="https://cococarvan.vercel.app">Visit site</a>
    </td>
    <td width="50%" valign="top">
      <h3>🌊 Marlin</h3>
      <p>A TypeScript product experiment deployed to the web—part of an expanding portfolio of shipped ideas.</p>
      <p><strong>TypeScript • Web Product Engineering</strong></p>
      <a href="https://github.com/rasesh13/marlin">Source</a> ·
      <a href="https://marlin-delta.vercel.app">Live experience</a>
    </td>
  </tr>
</table>

## Engineering signal

<div align="center">

<img width="98%" src="https://github-profile-trophy.vercel.app/?username=rasesh13&theme=tokyonight&no-frame=true&no-bg=true&margin-w=12&margin-h=12&column=7" alt="Rasesh's GitHub trophies" />

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=rasesh13&show_icons=true&hide_border=true&theme=tokyonight&rank_icon=github&include_all_commits=true" alt="Rasesh's GitHub statistics" />
<img width="49%" src="https://streak-stats.demolab.com?user=rasesh13&theme=tokyonight&hide_border=true" alt="Rasesh's GitHub contribution streak" />

<img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=rasesh13&theme=tokyo-night&hide_border=true&area=true&custom_title=Contribution%20Velocity" alt="Rasesh's animated contribution graph" />

</div>

## Contribution arcade

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/rasesh13/rasesh13/gh-pages/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/rasesh13/rasesh13/gh-pages/github-contribution-grid-snake.svg" />
  <img width="98%" alt="Animated contribution snake" src="https://raw.githubusercontent.com/rasesh13/rasesh13/gh-pages/github-contribution-grid-snake.svg" />
</picture>

<sub>The snake redraws automatically every day from live contribution data.</sub>

</div>

## How I build

```text
Discover  →  Design  →  Prototype  →  Engineer  →  Test  →  Ship  →  Learn  ↻
```

> Great software is not just code that works. It is an experience people trust, enjoy, and return to.

<div align="center">

### Let’s build something people remember.

<a href="https://github.com/rasesh13?tab=repositories">
  <img src="https://img.shields.io/badge/EXPLORE_MY_WORK-0D1117?style=for-the-badge&logo=github&logoColor=white" alt="Explore my repositories" />
</a>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:FF0080,50:7F00FF,100:00C9FF&height=120&section=footer&animation=fadeIn" alt="Animated footer" />

</div>
